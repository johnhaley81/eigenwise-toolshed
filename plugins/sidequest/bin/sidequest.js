...
const ARRAY_FLAGS = /* @__PURE__ */ new Set(["image", "label", "file", "add-file", "remove-file", "resolved-path", "always-in-scope", "read-only-denied-tool", "auto-approve-scope", "produces", "changes", "consumes", "changed-surface", "dependency"]);
...
const COMMAND_FLAGS = {
  ...
  models: ["full"],
  route: ["ticket"],
  "board-config": ["name", "always-in-scope", "read-only-denied-tool", "generated-pairs", "integration-mode", "integration-branch", "delivery", "integration-verify-timeout-ms", "worktree-isolation", "worktree-base", "not-integrated-salvage-age-hours", "worktree-recovery-retention-age-hours", "worktree-budget-max-count", "worktree-budget-max-bytes", "auto-approve-test-scope", "auto-approve-scope", "worktree-setup", "worktree-dependency-paths"],
  projects: ["archived"],
...
function commandMutates(command, opts, positional) {
  ...
  if (command === "board-config" || command === "board_config") {
    return ["name", "always-in-scope", "read-only-denied-tool", "generated-pairs", "integration-mode", "integration-branch", "worktree-isolation", "worktree-base", "not-integrated-salvage-age-hours", "worktree-recovery-retention-age-hours", "worktree-budget-max-count", "worktree-budget-max-bytes", "auto-approve-test-scope", "auto-approve-scope", "worktree-setup", "worktree-dependency-paths"].some((key) => Object.hasOwn(opts, key));
  }
  return false;
}
